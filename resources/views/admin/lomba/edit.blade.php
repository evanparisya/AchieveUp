...
<div class="container mx-auto p-4 max-w-lg">
    ...
    <form id="form-edit" action="{{ url('admin/lomba/update/' . $lomba->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <input type="date" name="tanggal_daftar_terakhir" id="tanggal_daftar_terakhir" class="w-full mb-3 p-2 border rounded"
        ...
        <select name="tingkat" id="tingkat" class="w-full mb-3 p-2 border rounded">
            ...
            @foreach (['internasional', 'nasional', 'regional', 'provinsi'] as $tingkat)
                <option value="{{ $tingkat }}" {{ old('tingkat', $lomba->tingkat) == $tingkat ? 'selected' : '' }}>{{ ucfirst($tingkat) }}</option>
            @endforeach
        </select>
        @error('tingkat')
            <p class="text-red-500 text-sm mb-2">{{ $message }}</p>
        ...
        @if ($lomba->file_poster)
            <p class="text-sm text-gray-600 mb-2">File saat ini: <a href="{{ asset('storage/' . $lomba->file_poster) }}" target="_blank" class="text-blue-600 underline">Lihat Poster</a></p>
        @endif
        @error('file_poster')
            <p class="text-red-500 text-sm mb-2">{{ $message }}</p>
        @enderror
