...
@section('content')
    <div class="container mx-auto px-4 py-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    <label for="tanggal_selesai" class="block text-sm font-medium text-gray-700 mb-1">Tanggal
                        Selesai</label>
                    <input type="date" name="tanggal_selesai" id="tanggal_selesai" required class="input"
                ...
                <div>
                    ...
                    <select name="juara" id="juara" required class="input">
                        ...
                        <option value="1" {{ old('juara') == 1 ? 'selected' : '' }}>1</option>
                        <option value="2" {{ old('juara') == 2 ? 'selected' : '' }}>2</option>
                        <option value="3" {{ old('juara') == 3 ? 'selected' : '' }}>3</option>
                    </select>
                </div>
                ...
                <div>
                    <label for="bidang" class="block text-sm font-medium text-gray-700 mb-1">Bidang Prestasi</label>
                    <select name="bidang" id="bidang" required class="input">
        ...
        "
                        ...
                        <span id="foto_kegiatan_label" class="mb-2 text-gray-500 select-none" style="cursor:pointer;"
                            x-show="!fileName">Drag & drop file di sini atau</span>
                        <button type="button" class="px-4 py-2 bg-[#6041CE] text-white rounded shadow text-sm mb-2"
                            style="cursor:pointer;" x-show="!fileName">Pilih File</button>
                        <input type="file" name="foto_kegiatan" id="foto_kegiatan" accept="image/jpeg,image/png"
                            class="hidden" x-ref="fileInput" @change="handleFileChange($event)" />

                        <template x-if="fileName">
                            <div class="flex items-center gap-2 mt-3 bg-gray-100 rounded px-3 py-2 w-full max-w-xs">
                                <template x-if="fileUrl">
                                    <img :src="fileUrl" alt="Preview" class="h-8 w-8 object-cover rounded" />
                                </template>
                                <span class="truncate flex-1 text-sm" x-text="fileName"></span>
                    ...
                    updateFileLabel('foto_kegiatan');
                "
                                    class="text-red-500 hover:text-red-700" title="Hapus" style="cursor:pointer;">
                ...
                <div x-data="{
                    ...
                    <label for="file_poster" class="block text-sm font-medium text-gray-700 mb-1">File Poster (jpg/png,
                        optional)</label>
        ...
        "
                        style="cursor: pointer;">
                        <span id="file_poster_label" class="mb-2 text-gray-500 select-none" style="cursor:pointer;"
                            x-show="!fileName">Drag & drop file di sini atau</span>
                        <button type="button" class="px-4 py-2 bg-[#6041CE] text-white rounded shadow text-sm mb-2"
                            style="cursor:pointer;" x-show="!fileName">Pilih File</button>
                ...
                <div>
                    <label for="nomor_surat_tugas" class="block text-sm font-medium text-gray-700 mb-1">Nomor Surat
                        Tugas</label>
                    <input type="text" name="nomor_surat_tugas" id="nomor_surat_tugas" required class="input"
                        value="{{ old('nomor_surat_tugas') }}">
                </div>

                <div>
                    <label for="tanggal_surat_tugas" class="block text-sm font-medium text-gray-700 mb-1">Tanggal Surat
                        Tugas</label>
                    <input type="date" name="tanggal_surat_tugas" id="tanggal_surat_tugas" required class="input"
                ...
                <div x-data="{
                    ...
                    <label for="file_surat_tugas" class="block text-sm font-medium text-gray-700 mb-1">File Surat Tugas
                        (.pdf)</label>
                ...
                event.currentTarget.classList.remove('border-primary');
            ">
                        <span id="file_surat_tugas_label" class="mb-2 text-gray-500" x-show="!fileName">Drag & drop file
                            di sini atau</span>
                        <button type="button" class="px-4 py-2 bg-[#6041CE] text-white rounded shadow text-sm"
                            x-show="!fileName">Pilih File</button>
                        <input type="file" name="file_surat_tugas" id="file_surat_tugas" accept="application/pdf"
                            required class="hidden" x-ref="fileInput" @change="handleFileChange($event)" />
                ...
                <div x-data="{
                    ...
                    <label for="file_sertifikat" class="block text-sm font-medium text-gray-700 mb-1">File Sertifikat
                        (.pdf)</label>
            ...
            "
                        style="cursor: pointer;">
                        <span id="file_sertifikat_label" class="mb-2 text-gray-500 select-none" style="cursor:pointer;"
                            x-show="!fileName">Drag & drop file di sini atau</span>
                        <button type="button" class="px-4 py-2 bg-[#6041CE] text-white rounded shadow text-sm mb-2"
                            style="cursor:pointer;" x-show="!fileName">Pilih File</button>
                        <input type="file" name="file_sertifikat" id="file_sertifikat" accept="application/pdf"
                            required class="hidden" x-ref="fileInput" @change="handleFileChange($event)" />
                ...
                <div x-data="{
                    ...
                    filtered() {
                        return {{ json_encode($mahasiswas) }}.filter(item => {
                            return (
                    ...
                    <input type="text" x-model="query" placeholder="Cari nama atau NIM..." class="input">

                    <div class="mt-2 flex flex-wrap gap-2">
                        <template x-for="item in selected" :key="item.id">
                            <span
                                class="inline-flex items-center bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm pl-[20px] relative">
                                <span x-text="item.nama + ' (' + item.nim + ')'"></span>
                                <button type="button" :disabled="item.id === {{ $mhs->id }}"
                                    :class="[
                                        'flex h-[32px] w-[32px] items-center justify-center rounded-full text-red-500 transition hover:text-red-700 focus:outline-none ml-2',
                                        item.id === {{ $mhs->id }} ? 'cursor-not-allowed opacity-70' :
                                        'cursor-pointer'
                                    ]"
                                    @click="selected = selected.filter(i => i.id !== item.id)" title="Hapus">
                    ...
                    <ul x-show="query.length > 0 && filtered().length > 0"
                        ...
                        <template x-for="item in filtered()" :key="item.id">
                            <li @click="selected.push(item); query = ''"
                ...
                <div x-data="{
                    ...
                    selected: [],
                    filtered() {
                        return {{ json_encode($dosens) }}.filter(item => {
                            return (
                                item.nama.toLowerCase().includes(this.query.toLowerCase()) ||
                    ...
                    <input type="text" x-model="query" placeholder="Cari nama atau NIDN..." class="input">

                    <div class="mt-2 flex flex-wrap gap-2">
                        <template x-for="item in selected" :key="item.id">
                            <span
                                class="inline-flex items-center bg-green-100 text-green-800 px-3 py-1 rounded-full text-sm pl-[20px] relative">
