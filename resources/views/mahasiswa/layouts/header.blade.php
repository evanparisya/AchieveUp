<header class="fixed top-0 left-0 right-0 z-50 bg-white shadow px-8 py-4 flex items-center justify-between">
    ...
    <div class="flex items-center space-x-3 justify-end relative">
        <div class="flex flex-col text-right leading-tight">
            <span class="text-md font-medium text-gray-800">{{ Auth::guard('mahasiswa')->user()->nama ?? 'Nama Pengguna' }}</span>
            <span class="text-xs text-gray-500">{{ Auth::guard('mahasiswa')->user()->nim ?? '-' }}</span>
        </div>
        <div x-data="{ open: false }" class="relative">
            <img
                src="{{ Auth::guard('mahasiswa')->user() && Auth::guard('mahasiswa')->user()->foto ? asset(Auth::guard('mahasiswa')->user()->foto) : asset('images/default-user.png') }}"
                ...
                <div class="px-4 py-3 border-b">
                    <div class="flex items-center gap-3 mb-2">
                        <img
                            ...
                            onerror="this.onerror=null;this.src='{{ asset('images/default-user.png') }}';"
                            alt="User Image"
                            class="w-10 h-10 rounded-[12px] object-cover border-2 border-primary">
                        <div>
                            <div class="font-semibold text-sm text-gray-800 truncate">{{ Auth::guard('mahasiswa')->user()->nama ?? '-' }}</div>
                            <div class="text-xs text-gray-500 truncate">{{ Auth::guard('mahasiswa')->user()->email ?? '-' }}</div>
                        </div>
                    </div>
                </div>
