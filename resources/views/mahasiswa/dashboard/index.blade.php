        ...
        <div class="lg:col-span-4 flex flex-col h-auto lg:h-[548px] p-6 items-center gap-6 rounded-xl bg-white shadow-md">
            ...
            <div class="flex justify-between items-center self-stretch">
                ...
                <a href="{{ route('mahasiswa.prestasi.index') }}"
                    class="text-[#6041CE] text-right text-sm font-semibold leading-normal">
                    Lihat Semua
                </a>
            </div>
            ...
            <div class="flex flex-col justify-center items-center gap-2 self-stretch">
                @if ($prestasiTerakhir && $prestasiTerakhir->count() > 0)
                    @foreach ($prestasiTerakhir->take(3) as $prestasi)
                        <div
                            class="flex items-center gap-2.5 flex-1 self-stretch rounded-[12px] border border-[rgba(0,0,0,0.1)] bg-[rgba(222,197,252,0.1)] p-2.5 mb-3 w-full">

                            <div class="flex justify-between items-center flex-1">

                                <span class="text-[#414651] text-md font-medium leading-normal truncate pr-2">
                                ...
                                <div class="text-right flex-shrink-0">
                                    @if ($prestasi->juara)
                                        <span class="block text-sm text-gray-600 whitespace-nowrap">
                                            Juara: {{ $prestasi->juara }}
                                        </span>
                                    @endif
                                    <span class="block text-xs text-gray-500 whitespace-nowrap">
                                        {{ \Carbon\Carbon::parse($prestasi->tanggal_selesai)->locale('id')->isoFormat('dddd, D MMMM YYYY') }}
                                    </span>
                                </div>
                            </div>
                        </div>
                ...
                @else
                    <div
                        class="flex items-center gap-2.5 flex-1 self-stretch rounded-[12px] border border-[rgba(0,0,0,0.1)] bg-[rgba(222,197,252,0.1)] p-2.5 w-full">
        ...
        <div class="lg:col-span-8 flex flex-col items-start gap-6 ">
            <div
                ...
                <div
                    ...
                    <p class="text-[28px] font-bold">{{ $totalPrestasiDiajukan }}</p>
                </div>
                <div
                    ...
                    <p class="text-[28px] font-bold">{{ $totalPrestasiDisetujui }}</p>
                </div>
                <div
                    ...
                    <p class="text-[28px] font-bold">{{ $totalPrestasiDitolak }}</p>
                </div>
            </div>
